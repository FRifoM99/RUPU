...
// Variables para evitar conflicto con PID (NO USAR clearCount)
long prev_count_der = 0;
long prev_count_izq = 0;
...
float calcularNivelSensor(byte index, bool frontal) {
  ...
  if (sensorIndex < 8 ) return (nivel < UMBRAL_RUIDO_FRONTAL ) ? 0 : nivel;
  else return (nivel < UMBRAL_RUIDO_POSTERIOR ) ? 0 : nivel;
}
...
float getPosition(bool frontal){
  ...
  if(areaTotal == 0){ parar ="si"; return 0; }
  else { return sumPonderada / areaTotal; }  
}

void calibrarSensores() {
  for (int i = 0; i <= 15; i++) { sensoresMax[i] = 0; sensoresMin[i] = 4095; }
  
  constexpr int vel_cal = 200;
  encoder_der.clearCount();
  encoder_izq.clearCount();
  prev_count_der = 0; // Reiniciar variables locales de odometría
  prev_count_izq = 0;
  ...
  while(calcularNivelSensor(SENSOR_CENTRAL_FRONTAL, true) < 80){ 
    ...
    if (fabs(anguloGirado) > 2*PI) break;
  }                                                                                                          
  motor(0,0);
}
...
void Calculo_Velocidades() {
  ...
  if(t_actual>=10){
    for(int k=4; k>0; k--) { v_r[k] = v_r[k-1]; v_l[k] = v_l[k-1]; }
    
    // CORRECCIÓN: Usamos delta en lugar de clearCount() para no romper el PID
    long curr_der = encoder_der.getCount();
    long curr_izq = encoder_izq.getCount();
    long delta_der = curr_der - prev_count_der;
    long delta_izq = curr_izq - prev_count_izq;
    
    prev_count_der = curr_der;
    prev_count_izq = curr_izq;

    v_r[0]=2*PI*RADIO_RUEDA*delta_der*N_REDUCCION/(t_actual*0.001*CPR);
    v_l[0]=2*PI*RADIO_RUEDA*delta_izq*N_REDUCCION/(t_actual*0.001*CPR); 
...
void Curvatura_Pista() {
  ...
  if(Input_vel<0.1){
    ...
    b_curvatura=abs((0.5*(b_sr-b_sl))/(DIST_RUEDAS*b_s)); b_sl=0; b_sr=0;
  }
  else if(a_s>=3 && a_s==b_s){ b_sl=0; b_sr=0; }
  else if (a_s>=6 && c_s>=10){
    c_curvatura=abs((0.5*(c_sr-c_sl))/(DIST_RUEDAS*c_s)); c_sl=0; c_sr=0;
  }
  else if(a_s>=6 && a_s==c_s){ c_sl=0; c_sr=0; }
  else if(a_s>=10){
    a_curvatura=abs((0.5*(a_sr-a_sl))/(DIST_RUEDAS*a_s)); a_sl=0; a_sr=0;
  }
  
  if(a_curvatura<0.01 && b_curvatura<0.01 && c_curvatura<0.01) recta=1;
  else if(a_curvatura>0.02 && b_curvatura>0.02 && c_curvatura>0.02) recta=0;
