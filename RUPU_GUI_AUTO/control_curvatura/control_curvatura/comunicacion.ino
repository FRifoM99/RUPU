...
String configuracion_remota(int len) {
  ...
  if (variable == "/co_p") {
    ...
    EtiquetaRobot = dato;                          // "L","S","T","O" o lo que envíe la GUI
    IP_monitoreo = udp.remoteIP();      // aprende la IP del monitor 
    ack_recibido_recon = true;                     // Ha llegado el ACK de la GUI
  }
  else {
    return "incorrecto";
  }
