//Nueva version, modo automatico en reconocimiento de IPs
#include <esp_wifi.h>
#include <WiFi.h>
...
IPAddress IP_monitoreo(0,0,0,0);  //ip del dispositivo que va a supervisar los datos 
#define puerto_monitoreo 1234           //puerto de envio UDP monitoreo
IPAddress IP_sucesor(172,20,10,100);  //ip del robot sucesor
#define puerto_sucesor 1111          //puerto del robot sucesor
#define puerto_local 1111               //puerto local
////////
String EtiquetaRobot = "X";   // "L": Robot0 (lider)   "S": robot2    "T": Robot3    "O": Robot4   "X" Sin valor
#define led 2
...
//NUEVO estado
#define reconocimiento 3
// --- Timers y flags para reconocimiento ---
...
void loop(){
  Serial.print("ESTADO   :"); //prints debug
  Serial.println(estado);
  Serial.println(IP_monitoreo);
  Serial.println(EtiquetaRobot);
  Serial.println("IP sucesor:");
  Serial.println(IP_sucesor);
  switch (estado) {
    case inicio:
      ciclo_de_inicio();
      break;
  ...
  else if (estado == inicio && disparo_sensor_boton() && !ack_recibido_recon) {
    estado_siguiente = reconocimiento;             // entrar por sensor y si NO hay ACK
  }
  else if (estado == reconocimiento && !ack_recibido_recon) {
    estado_siguiente = reconocimiento;             // mantenerse hasta que llegue ACK
  }
  else if (estado == reconocimiento && ack_recibido_recon) {
    estado_siguiente = inicio;                     // salir cuando llega ACK
  }
