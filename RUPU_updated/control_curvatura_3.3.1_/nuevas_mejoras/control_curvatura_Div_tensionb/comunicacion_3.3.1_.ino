...
String configuracion_remota(int len) {
  ...
  if (variable == "/co_p") {
    ...
    //parar = dato;
    if (dato == "si") flag_parar = true;
    else flag_parar = false;
  }
  else {
    return "incorrecto";
  }
...
void udp_transm() {
  t_actual = millis() - t_com_predecesor;
  if (t_actual >= 100) {
    cadena = "V/" + parar + "/" + String(flag_parar ? "si" : "no") + "/" + String(vel_ref) + "/" + String(curvatura);
    cadena.toCharArray(msg, cadena.length() + 1);
    udp.beginPacket(MONITOR_IP, MONITOR_PORT);
    udp.printf(msg);
    udp.endPacket();
    t_com_predecesor = millis();
  }
...
void udp_monitor() {
  ...
  cadena.toCharArray(msg, cadena.length() + 1);
  //cambie aqui las variables, IP_monitoreo, puerto_monitoreo
  udp.beginPacket(MONITOR_IP, MONITOR_PORT);
  udp.printf(msg);      //
  udp.endPacket();
  //  t_monitoreo = millis();
